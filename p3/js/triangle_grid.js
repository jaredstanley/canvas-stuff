...
function drawItem(){
	...
	// contxt.arc(600*Math.random(), size*(icount%itemsInRow), size, 0, Math.PI*2, true); 	
	// contxt.fill();
...
function drawItemsInCircle(){
	 numberOfPoints = 90;
	 circleRadius = 200*Math.random();
	 angleIncrement = 360 / numberOfPoints; 
	// contxt.fillStyle= randColor(colorInc%360, 1, 1, 1);	
	for (var i = 0; i < numberOfPoints; i+=1) {
	contxt.fillStyle= randColor(angleIncrement*i, (circleRadius/200)-.15, 1, 1);	
     circleRadius+= ( (Math.random()-Math.random())*20 );
		...
		contxt.beginPath();
		contxt.arc(xx, yy, size*.3, 0, Math.PI*2, true); 
		//
